...
function SalesDirectLead() {
    const [DirectleadsFromJr , setDirectleadsFromJr] = useState([])
        ...
        return onSnapshot(q, (querySnapshot) => {
            querySnapshot.forEach((doc) => {
                ...
                const DataQuery = query(collection(db, "Trip"),
                    ...
                    where("InstaId","==","Direct lead"),
                    where("assigned_date_time", ">=", firstDay));
                getConvertedDataForUserProfile(usersProfile, DataQuery, setDirectleadsFromJr);
            });
        });
    }
    useEffect(() => {
    ...
    return (
        <div>
            <Verticlechart Data={DirectleadsFromJr} Comment={`Sales Direct Leads ${moment(firstDay).format('MMMM')}`}/>
        </div>
    );
}

export default SalesDirectLead;
