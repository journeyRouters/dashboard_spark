...
function Last3rdMonthDumpLead() {
    const [DumpLeads, setDumpLeads] = useState([]);

    // Get the start and end of the previous month
    const startOfPrevMonth = moment().subtract(2, 'month').startOf('month').toDate();
    const endOfPrevMonth = moment().subtract(2, 'month').endOf('month').toDate();
    const prevMonthName = moment().subtract(2, 'month').format('MMMM');
...
export default Last3rdMonthDumpLead;
